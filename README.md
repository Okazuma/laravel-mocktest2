...
## 機能一覧
    ...
    6 お気に入りの登録と取り消し機能(イイね♡押下)

    7 飲食店への予約機能(日付、時間、人数の指定可能)

    8 メニュー画面の切り替え

    9 飲食店代表者の作成 (管理者権限)

    10 CSVファイルのインポート機能 (管理者権限)

    11 飲食店情報の追加、編集 (飲食店代表者権限)

    12 各飲食店の予約情報の予約情報の確認 (飲食店代表者権限)

    13 ユーザーへのお知らせメール機能 (飲食店代表者権限)

    14 利用者へ予約日当日の午前9:00にリマインダーメールを送信

    15 stripe決済機能

    16 QRコードで予約情報の確認 (飲食店代表者権限)

    17 飲食店に対しての口コミ機能(レビュー)
